...
def return_book(book_id):
    ...
    c.execute("""
        UPDATE books 
        SET status='available', borrower=NULL, borrowed_date=NULL 
        WHERE id=?
    """, (book_id,))
    conn.commit()
    conn.close()
...
with st.form("book_form"):
    ...
    year = st.number_input("Tahun", min_value=0, max_value=3000, value=year_default)
...
if rows:
    df = pd.DataFrame(rows, columns=[
        "ID","Judul","Penulis","Tahun","ISBN","Status","Peminjam","Tanggal Pinjam"
    ])
